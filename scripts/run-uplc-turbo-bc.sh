...
UPLC_BENCH_MODE=bytecode "$BENCH_BIN" --bench 2>&1 | tee "$RUN_DIR/uplc-turbo-bc-raw.log" || true

# Collect failed scripts
FAILED_SCRIPTS=$(grep "^EVAL_FAIL:" "$RUN_DIR/uplc-turbo-bc-raw.log" | sed 's/EVAL_FAIL: //' | sort -u || true)

# Copy Criterion JSON output (remove old copy first to avoid cp -r nesting)
rm -rf "$RUN_DIR/criterion-output-bc"
cp -r target/criterion "$RUN_DIR/criterion-output-bc" 2>/dev/null || true

# Parse criterion output, filtering out scripts that failed evaluation
python3 /bench/parsers/parse_criterion.py "$RUN_DIR/criterion-output-bc" "uplc-turbo-bc" > "$RUN_DIR/uplc-turbo-bc-raw.csv"

# Filter: remove bogus timings for failed scripts, then add -1 entries
python3 -c "
import sys
failed = set('''$FAILED_SCRIPTS'''.split())
with open('$RUN_DIR/uplc-turbo-bc-raw.csv') as f:
    lines = f.readlines()
with open('$RUN_DIR/uplc-turbo-bc.csv', 'w') as out:
    ...
    for script in sorted(failed):
        if script:
            out.write(f'uplc-turbo-bc,{script},-1,-1,-1,-1,-1,0\n')
"

rm -f "$RUN_DIR/uplc-turbo-bc-raw.csv"

# Fill in -1 for any scripts that were given but produced no result
python3 /bench/parsers/fill_failures.py "$RUN_DIR/uplc-turbo-bc.csv" "$DATA_DIR" uplc-turbo-bc "$RUN_DIR/uplc-turbo-bc-raw.log"
